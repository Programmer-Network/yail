...
const Alert: FC<AlertProps> = ({ type, children, className }) => {
  return (
    <div className='flex min-h-screen max-w-(--breakpoint-sm) items-center justify-center'>
      <div
        className={classNames("bg-linear-to-r p-[1px]", className, {
          "from-green-400 via-green-500 to-green-900 text-green-500":
            type === "success",
          "from-red-400 via-error to-red-900 text-error": type === "error",
      ...
      >
        <div className='back flex h-full w-full items-center justify-center bg-background p-4'>
          <span>{children}</span>
        </div>
      </div>
    </div>
